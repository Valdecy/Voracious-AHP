...
public class Logic_MeanNormalization {

	public static double mn_ConsistencyRatio (double[][] array){

		double[][] arrayN  = MatrixOperations.get2DNormalized(array);
		double[][] arrayW = new double[array.length][array[0].length];
		double  [] weigths = MatrixOperations.get2DColumnSumArray(MatrixOperations.get2DTransposedMatrix(arrayN));
		
		double number_of_criteria = (array[0].length*1d)/1.0;
		for (int i = 0; i < array[0].length; i++){
			weigths[i] = weigths[i]/number_of_criteria;
		}
